...
class BlipProjModel(torch.nn.Module):
    """Projection Model 768->1024 """

    def __init__(self, input_embeddings_dim=768, output_attention_dim=1024):
        super().__init__()

        self.generator = None
        self.proj = torch.nn.Linear(input_embeddings_dim, output_attention_dim)
...
class SGAdapter:
    def __init__(self, sd_pipe, image_encoder_path, sg_ckpt, device, num_tokens=4, scale=1.0):
        self.device = device
...
class SGAdapterFull(SGAdapterPlus):
...
class SGAdapterPlusXL(SGAdapter):
    """SDXL"""
