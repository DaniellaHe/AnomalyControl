...
def test(args,obj_name, model_seg):
    ...
    if args.dataset == "mvtec_ad":
        ...
        dataset = VISATestDataset_partial(dataset_path + '/' + obj_name + "/test/",
                                              resize_shape=[img_dim, img_dim])
    elif args.dataset == "mpdd":
        dataset = MPDDTestDataset_partial(dataset_path + '/' + obj_name + "/test/",
                                          resize_shape=[img_dim, img_dim])
...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        if args.dataset=="mvtec_ad":
            ...
            dataset = VISA_Anomaly_Detection(args,obj_name,length=500)
        elif args.dataset=="mpdd":
            dataset = MPDD_Anomaly_Detection(args,obj_name,length=500)
        ...
        for epoch in range(args.epochs):
            ...
            if sum_metric>last_sum:
                torch.save(model_seg.state_dict(), os.path.join(save_path, run_name + ".pckl"))
                last_sum=sum_metric
...
if __name__=="__main__":
    ...
    obj_batch = [name for name in os.listdir(args.generated_data_path) 
                if os.path.isdir(os.path.join(args.generated_data_path, name))]
    

    obj_batch = [obj for obj in obj_batch if obj not in ['.', '..']]
