...
class VISATestDataset_partial(Dataset):
...
class MPDDTestDataset_partial(Dataset):
    ...
    def transform_image(self, image_path, mask_path):
        ...
        if mask_path is not None:
            mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE | cv2.IMREAD_IGNORE_ORIENTATION)
        else:
            mask = np.zeros((image.shape[0],image.shape[1]))
...
class MVTec_Anomaly_Detection(Dataset):
    def __init__(self, args,sample_name,length=5000,anomaly_id=None,recon=False):
        ...
        if args.dataset=='visa':
            self.good_path='%s/%s/train'%(args.dataset_path,sample_name)
        else:
            self.good_path='%s/%s/train/good'%(args.dataset_path,sample_name)
...
class VISA_Anomaly_Detection(Dataset):
    def __init__(self, args,sample_name,length=5000,anomaly_id=None,recon=False):
        self.recon=recon
        self.good_path='%s/%s/train'%(args.dataset_path,sample_name)
        self.good_files=[os.path.join(self.good_path,i) for i in os.listdir(self.good_path)]
        ...
        for idx,anomaly in enumerate(self.anomaly_names):
            ...
            mask_path=[]
            for i in range(min(len(os.listdir(os.path.join(self.root_dir,anomaly,'mask'))),500)):
                img_path.append(os.path.join(self.root_dir,anomaly,'image',f'{sample_name}_{anomaly}_{i+1}.png'))
                mask_path.append(os.path.join(self.root_dir,anomaly,'mask',f'{sample_name}_{anomaly}_{i+1}.png'))
            self.img_paths.append(img_path.copy())
            self.mask_paths.append(mask_path.copy())
        for i in range(l):
            print(len(self.img_paths[i]),len(self.mask_paths[i]))
...
class MPDD_Anomaly_Detection(Dataset):
    def __init__(self, args,sample_name,length=5000,anomaly_id=None,recon=False):
        self.recon=recon
        self.good_path='%s/%s/train/good'%(args.dataset_path,sample_name)
        self.good_files=[os.path.join(self.good_path,i) for i in os.listdir(self.good_path)]
...
class MVTec_classification_test(Dataset):
    def __init__(self, args,sample_name,anomaly_names):
        root_dir = '%s/%s/test'%(args.dataset_path,sample_name)
